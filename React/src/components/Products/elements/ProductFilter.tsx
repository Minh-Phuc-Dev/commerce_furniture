...
const RangePrice = () => {
    ...
    const [value, setValue] = useState<number[]>([currentMinPrice, currentMaxPrice]);
    const [min, max] = value
        ...
        <div className="space-y-2">
            ...
            <div>
                <div className="flex items-center gap-2">
                    <RangeSlider min={0} max={20000000} value={value as [number, number]} onInput={setValue} />
                </div>
            ...
            <ul>
                {
                    categories.data.map(
                        category => (
                            <li
                                key={category.id}
                                className="group flex items-center gap-x-1.5 text-gray-600 hover:text-gray-900 cursor-pointer"
                                data-category={category.id}
                                onClick={onCategoryClick}
                            >
                                <button

                                    className="block size-4 rounded border-gray-600 border-1.5"
                                    ...
                                    <svg
                                        ...
                                        className={
                                            twMerge(
                                                ...
                                                currentCategory.includes(category.id.toString()) ? "block" : "hidden"
                                            )
                                        }
                                    >
...
export const SortBy: FC<HTMLAttributes<HTMLDivElement>> = ({ className, ...props }) => {
    const [searchParams, setSearchParams] = useSearchParams();
    ...
    return (
        ...
        >
            <button
                className="peer focus:border-primary flex items-center justify-between text-left w-40 rounded border px-2 py-0.5 peer"
            >
                <span>
                    {
                        isEmpty(sortByPrice) ? "Sắp xếp theo giá" : sortByPrice === "desc" ? "Giá Cao - Thấp" : "Giá Thấp - Cao"
                    }

                </span>
                {
                    sortByPrice === "desc" ? (
                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" className="size-4 fill-primary">
            ...
            <ul
                className="absolute hidden z-20 top-full w-full py-2 mt-2 rounded bg-white border peer-focus:block hover:block"
            >
                <li
                    data-sort="asc"
...
const ProductFilter: FC<HTMLAttributes<HTMLDivElement>> = ({ className, ...props }) => {
    return (
        <div
            className={
                twMerge(
